...
<form name="formEntrada" id="formEntrada" method="post" action="" class="" onSubmit="return valida_form(this)">
    ...
    <input type="text" name="cliente" id="cliente" size="40" maxlength="9" value="" alt="valida" class="md" autofocus  required/>     
    </p>
    <br />
...
function buscaConta(){
	...
	$.get(
		"ajax3.php",
		{ cliente: cliente, origem:"buscaconta" },
		function(data){
			if (data == 'erro'){
				alert('erro');
...
$(".btEstornar").live("click", function(){
	var id = $(this).attr('id');
	if (confirm("Estornar Recebimento?")){		
		estornaRecebimento(id);
	}
});


function estornaRecebimento(id){
	var id = id;
	$.get(
		"ajax3.php",
		{ codigo: id, origem:"estornaRecebimento" },
		function(data){
			if (data.erro == '1'){
				//alert('erro');
