...
function buscaPagar(){
	...
	$.get(
		"ajax3.php",
		{ cliente: cliente, origem:"buscaPagar" },
		function(data){
			if (data == 'erro'){
				alert('Não há contas'); //erro
...
function estornaPagar(id){
	var id = id;
	$.get(
		"ajax3.php",
		{ codigo: id, origem:"estornaPagar" },
		function(data){
			if (data.erro == '1'){
				alert('Não foi posível cancelar a conta'); //erro
